    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20 md:pb-0 md:pl-20">
            ...
            <nav className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-20 bg-white border-r border-slate-200 items-center py-8 gap-8 z-50">
                ...
                <NavLink to="/" className={({ isActive }) => `p-3 rounded-xl transition-colors ${isActive ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <Home size={24} />
                </NavLink>
                <NavLink to="/market" className={({ isActive }) => `p-3 rounded-xl transition-colors ${isActive ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <TrendingUp size={24} />
                </NavLink>
                <NavLink to="/search" className={({ isActive }) => `p-3 rounded-xl transition-colors ${isActive ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <Search size={24} />
                </NavLink>
                <NavLink to="/chats" className={({ isActive }) => `p-3 rounded-xl transition-colors ${isActive ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <MessageSquare size={24} />
                </NavLink>
                <NavLink to="/profile" className={({ isActive }) => `p-3 rounded-xl transition-colors ${isActive ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <User size={24} />
                </NavLink>
            </nav>
